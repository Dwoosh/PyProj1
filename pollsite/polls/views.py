...
class CommentView(generic.CreateView):
	model = Comment
	fields = ['comment_text']
	template_name = 'polls/comment_create_form.html'
	
	def form_valid(self, form):
		comment = form.save(commit=False)
		question_id = Question.objects.get(id=self.kwargs['question_id'])
...
def comment(request, question_id):
	question = get_object_or_404(Question, pk=question_id)
	try:
		question.comment_set.get(pk=request.POST['choice'])
